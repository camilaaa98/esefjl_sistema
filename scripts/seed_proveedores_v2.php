<?php
/**
 * Script para generar 50 proveedores - Versión 2 con SQLite3 nativo
 */

$dbPath = __DIR__ . '/../database/esefjl.db';
...
$stmt = $db->prepare("INSERT INTO proveedores (nit, razon_social, contacto, telefono, email) VALUES (:nit, :razon, :contacto, :tel, :email)");
...
foreach ($proveedores as $prov) {
    try {
        $stmt->bindValue(':nit', $prov[0], SQLITE3_TEXT);
        $stmt->bindValue(':razon', $prov[1], SQLITE3_TEXT);
        $stmt->bindValue(':contacto', $prov[2], SQLITE3_TEXT);
        $stmt->bindValue(':tel', $prov[3], SQLITE3_TEXT);
        $stmt->bindValue(':email', $prov[4], SQLITE3_TEXT);
        $stmt->execute();
