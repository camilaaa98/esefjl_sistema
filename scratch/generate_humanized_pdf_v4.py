...
Bonet, J. A., Ricciulli-Marín, D., Pérez-Valbuena, G. J., Galvis-Aponte, L. A., Haddad, E. A., Araújo, I. F., & Perobelli, F. S. (2020). Impacto económico regional del Covid-19 en Colombia. Banco de la República.
...
Organización Internacional del Trabajo [OIT]. (2020). América Latina y el Caribe frente a la pandemia del COVID-19: Efectos en la región. Panorama Laboral.
...
Weller, J. (2020). La pandemia del COVID-19 y su efecto en los mercados laborales sudamericanos. Informes Macro CEPAL.
Vargas, M. (2021). Impacto de los alivios financieros gubernamentales en la Amazonía. Revista de Hacienda Pública, 28, 44-59."""


class PDF(FPDF):
    def header(self):
        # Empty header so title is not repeated.
        pass
...
# APA 7 margins: 2.54 cm on all sides (1 inch = 25.4 millimeters)
pdf.set_margins(left=25.4, top=25.4, right=25.4)

pdf.add_page()

# Enable auto page break matching the bottom margin
...
# Simulate hanging indent for bibliography by shifting X position after first line.
# In fpdf2 it's easiest to just print it as multi_cell since handling the hanging indent manually is tricky.
...
print("Archivos PDFs generados con mrgenes APA y tipografa uniforme")
